...
def query = "DECLARE @parameter varchar(50); SET @parameter = (SELECT TOP 1 NO_REGISTRASI FROM [webapps]..DATAPENGAJUAN  WHERE STATUS IN('61') AND CONVERT(varchar, created_at, 101) = CONVERT(varchar, getdate(), 101) ORDER BY created_at DESC); update [JMPMFI_Mobile]..[MO_DataSurvey] set kode_cabang ='839'where no_registrasi = @parameter ;select top 1 * from [JMPMFI_Mobile]..[MO_DataSurvey] where no_registrasi = @parameter"
// Eksekusi query
